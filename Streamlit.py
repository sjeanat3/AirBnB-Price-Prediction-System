...
if st.button('Submit'):
    ...
    streamlit_model3_pipe = decompress_pickle('streamlit_model3_compress_pipe.pbz2')
#     with open('streamlit_model3_pipe.pkl', 'rb') as f:
#         streamlit_model3_pipe = pickle.load(f)
    
    checkin_features = user_input_checkin_features()
    checkout_features = user_input_checkout_features()

    checkin_price = streamlit_model3_pipe.predict(checkin_features)
    checkin_price = streamlit_model3_pipe.predict(checkin_features) 

    checkin_price = float(checkin_price)
    checkout_price = float(streamlit_model3_pipe.predict(checkout_features))
